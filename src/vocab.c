...
int AddWordToVocab(struct vocabulary *v, char *word) {
    verbose("        --> Adding %s to vocabulary\n", word);
    //static long collide = 0;
    //static long nocollide = 0;
    unsigned int hash, length = strlen(word) + 1; //account for null termination
    if (length >= MAX_STRING) { 
        verbose("     !!! Truncating word to first %d characters !!!\n", MAX_STRING-2);
        length = MAX_STRING-1; 
    }

    v->vocab[v->vocab_size].word = (char *)calloc(length+1, sizeof(char));
    if (!v->vocab[v->vocab_size].word) {
        error("   >>> Failed to allocate memory for new vocabulary word\n");
        exit(1);
    }

    // truncate the word at MAX_STRING if needed
    strncpy(v->vocab[v->vocab_size].word, word, length);
    v->vocab[v->vocab_size].word[length] = 0; //enforce null-terminated
    // set the observation frequency to 0
    v->vocab[v->vocab_size].cn = 0;
    v->vocab_size++;
    // Reallocate memory if needed
    if (v->vocab_size + 100 >= v->vocab_max_size) {
        v->vocab_max_size += 1000;
        v->vocab = (struct vocab_word *)realloc(v->vocab, v->vocab_max_size * sizeof(struct vocab_word));
        if (!v->vocab) {
            error("   >>> Failed to re-allocate memory for expanded vocabulary\n");
            exit(1);
        }
    }
...
// Increments a word's frequency count in the vocabulary
...
void SortAndReduceVocab(struct vocabulary *v, int min_count) {
    ...
    for (a = 0; a < size; a++) {
        ...
        if (v->vocab[a].cn < min_count) {
            reduced++;
            v->vocab_size--;
            if (v->vocab[v->vocab_size].word) free(v->vocab[v->vocab_size].word);
        } else {
    ...
    verbose("  >>> Reduced vocabulary size by %d words\n", reduced);
    v->vocab = (struct vocab_word *)realloc(v->vocab, (v->vocab_size + 1) * sizeof(struct vocab_word));
    if (!v->vocab) {
        error("   >>> Failed to re-allocate memory for reduced vocabulary\n");
        exit(1);
    }
}
...
struct vocabulary *CreateVocabulary() {
    ...
    if (!v) {
        error("   >>> Failed to allocate memory for vocabulary container\n");
        exit(1);
    }
    ...
    v->vocab = calloc(v->vocab_max_size, sizeof(struct vocab_word));
    if (!v->vocab) {
        error("   >>> Failed to allocate memory for vocabulary\n");
        exit(1);
    }
    // explicitly set all word pointers to NULL
    for (int i = 0; i < v->vocab_max_size; i++) {
        v->vocab[i].word = NULL;
    }
    
    v->vocab_hash = calloc(vocab_hash_size, sizeof(int));
    if (!v->vocab_hash) {
        error("   >>> Failed to allocate memory for vocab hash\n");
        exit(1);
    }
    for (a = 0; a < vocab_hash_size; a++) v->vocab_hash[a] = -1;
...
/**
 * Free and set to NULL all memory allocated for this vocabulary
 */
...
void SaveVocab(struct vocabulary *v, char *save_vocab_file) {
    ...
    // error check
    if (fo == NULL) {
        error("Unable to write vocabulary to %s, does the directory exist?", save_vocab_file);
        exit(1);
    }
    // write each vocab word with its frequency
...
int ReadCorpusWordIndex(struct vocabulary *v, FILE *fin) {
    ...
    else return SearchVocab(v, word);
}
...
int ReadWordIndex(struct vocabulary *v, FILE *fin) {
    ...
    if (feof(fin)) return -1;
    // ensure that -1 is returned if an optional read found no word
    if (word[0] == 0) return -1;
    else return SearchVocab(v, word);
}

struct vocabulary *ReadVocab(char *vocabfile) {
    long long a, i = 0, word_count = 0;
    char c;
    char word[MAX_STRING];
    FILE *fin = fopen(vocabfile, "rb");
    if (fin == NULL) {
        error("Vocabulary file %s not found\n", vocabfile);
        exit(1);
    }
    ...
    while (1) {
        ...
        word_count += v->vocab[a].cn;
        i++;
        verbose("   >>> Read %lld words...\r", i);
    }
    fclose(fin);
